...
exports.ChimeraStore = void 0;
exports.sessionPublicId = sessionPublicId;
exports.removeChimeraRuntimeForTests = removeChimeraRuntimeForTests;
...
class ChimeraStore {
    ...
    constructor(root) {
        this.root = root;
        (0, paths_1.ensureDir)((0, paths_1.chimeraSessionsDir)(root));
        this.db = new locked_sqlite_1.LockedSqliteDatabase((0, paths_1.chimeraDbPath)(root));
        this.configure();
        this.migrate();
    }
    close() {
    ...
    createSession(input) {
        ...
        try {
            ...
            const active = this.db.prepare("SELECT COUNT(*) AS count FROM chimera_sessions WHERE status IN ('starting', 'running')").get();
            ...
            const row = this.db.prepare("SELECT COALESCE(MAX(id), 0) + 1 AS next_id FROM chimera_sessions").get();
            const id = Number(row.next_id);
            const publicId = sessionPublicId(id);
            const sessionDir = node_path_1.default.join((0, paths_1.chimeraSessionsDir)(this.root), publicId);
            const labDir = node_path_1.default.join(sessionDir, "lab");
            (0, paths_1.ensureDir)(labDir);
            this.db.prepare(`
      ...
      `).run(id, publicId, input.role, input.goal, JSON.stringify(input.nodeIds), input.accessMode, input.accessNotes, input.model, input.variant, sessionDir, labDir, input.opencodeCommand, input.opencodeAgent, input.networkAllowed ? 1 : 0, input.autoApprove ? 1 : 0, input.serverUrl, now, now);
            this.db.exec("COMMIT");
    ...
    getSession(reference) {
        ...
        if (!row)
            throw new Error(`Unknown Chimera session: ${reference}`);
    ...
    updateSession(reference, input) {
        ...
        if (input.opencodeServerUrl !== undefined)
            add("opencode_server_url", input.opencodeServerUrl);
        if (input.opencodeSessionId !== undefined)
            add("opencode_session_id", input.opencodeSessionId);
        if (input.runPid !== undefined)
            add("run_pid", input.runPid);
        if (input.lastError !== undefined)
            add("last_error", input.lastError);
        if (input.stopped === true || input.status === "stopped")
            add("stopped_at", now);
        else if (input.status === "running" || input.status === "starting")
            add("stopped_at", null);
        values.push(current.id);
        this.db.prepare(`UPDATE chimera_sessions SET ${assignments.join(", ")} WHERE id = ?`).run(...values);
        return this.getSession(current.id);
    }
    claimSessionStart(reference) {
        this.db.exec("BEGIN IMMEDIATE");
        try {
            const current = this.getSession(reference);
    ...
    registerWorkerPid(reference, pid) {
    ...
    addMessage(input) {
        ...
        this.db.exec("BEGIN IMMEDIATE");
        try {
            const next = this.db.prepare("SELECT COALESCE(MAX(id), 0) + 1 AS next_id FROM chimera_messages").get();
      ...
      `).run(id, messagePublicId(id), session?.id ?? null, input.direction, input.fromId, input.toId, input.kind ?? "message", body, input.priority ? 1 : 0, input.toId === "coordinator" ? 0 : 1, input.toId === "coordinator" ? 1 : 0, now);
            this.db.exec("COMMIT");
    ...
    getMessage(reference) {
        ...
        if (!row)
            throw new Error(`Unknown Chimera message: ${reference}`);
    ...
    createCouncil(topicInput, participantReferences, maxRoundsInput = 2) {
        const topic = topicInput.trim();
        if (!topic)
            throw new Error("Council topic cannot be empty");
        const participants = [...new Set(participantReferences.map((reference) => this.getSession(reference).publicId))];
        if (participants.length === 0)
            throw new Error("Council requires at least one participant");
        const stopped = participants.filter((id) => this.getSession(id).status === "stopped");
        if (stopped.length > 0)
            throw new Error(`Council participants must be active: ${stopped.join(", ")}`);
        const now = new Date().toISOString();
        this.db.exec("BEGIN IMMEDIATE");
        try {
            const next = this.db.prepare("SELECT COALESCE(MAX(id), 0) + 1 AS next_id FROM chimera_councils").get();
            const id = Number(next.next_id);
            this.db.prepare(`
    ...
    getCouncil(reference) {
        ...
        if (!row)
            throw new Error(`Unknown Chimera council: ${reference}`);
        return toCouncil(row);
    }
    listCouncils(limitInput = 20) {
        return this.db.prepare("SELECT * FROM chimera_councils ORDER BY id DESC LIMIT ?").all(clampInteger(limitInput, 1, 200, 20)).map(toCouncil);
    }
    acceptCouncil(councilReference, participantReference) {
        const participant = this.getSession(participantReference).publicId;
        this.db.exec("BEGIN IMMEDIATE");
        try {
            const council = this.getCouncil(councilReference);
            if (council.status !== "inviting")
                throw new Error(`Council ${council.publicId} is not accepting invitations`);
            if (!council.participantIds.includes(participant))
                throw new Error(`${participant} is not invited to ${council.publicId}`);
    ...
    beginCouncil(councilReference, coordinatorOpening) {
        const body = coordinatorOpening.trim();
        if (!body)
            throw new Error("The coordinator opening cannot be empty");
        this.db.exec("BEGIN IMMEDIATE");
        try {
            const council = this.getCouncil(councilReference);
            if (council.status !== "inviting")
                throw new Error(`Council ${council.publicId} has already started or closed`);
            const missing = council.participantIds.filter((id) => !council.acceptedIds.includes(id));
            if (missing.length > 0)
                throw new Error(`Council is waiting for: ${missing.join(", ")}`);
    ...
    addCouncilTurn(councilReference, speakerReference, bodyInput) {
        const speaker = normalizeIdentity(speakerReference);
        if (speaker === "coordinator")
            throw new Error("Use council advance for a coordinator round opening");
        const body = bodyInput.trim();
        if (!body)
            throw new Error("Council turn cannot be empty");
        this.db.exec("BEGIN IMMEDIATE");
        try {
            const council = this.getCouncil(councilReference);
            if (council.status !== "open")
                throw new Error(`Council ${council.publicId} is not open`);
    ...
    advanceCouncil(councilReference, coordinatorOpening, extend = false) {
        const body = coordinatorOpening.trim();
        if (!body)
            throw new Error("The next-round opening cannot be empty");
        this.db.exec("BEGIN IMMEDIATE");
        try {
            const council = this.getCouncil(councilReference);
      ...
      `).run(nextRound, council.participantIds[0], extend && nextRound > council.maxRounds ? nextRound : council.maxRounds, new Date().toISOString(), council.id);
            this.db.exec("COMMIT");
            return { council: this.getCouncil(council.id), turn };
        }
        catch (error) {
    ...
    closeCouncil(councilReference, finalMessageInput) {
        const finalMessage = finalMessageInput.trim();
        if (!finalMessage)
            throw new Error("Council final message cannot be empty");
        this.db.exec("BEGIN IMMEDIATE");
        try {
            const council = this.getCouncil(councilReference);
    ...
    councilTurns(councilReference, limitInput = 100) {
        const council = this.getCouncil(councilReference);
    ...
    `).all(council.id, clampInteger(limitInput, 1, 1000, 100));
        return rows.map(toCouncilTurn);
    }
    insertCouncilTurn(council, round, speakerId, body) {
        const now = new Date().toISOString();
        const result = this.db.prepare(`
      INSERT INTO chimera_council_turns (council_id, round, speaker_id, body, created_at)
      VALUES (?, ?, ?, ?, ?)
    `).run(council.id, round, speakerId, body, now);
        return {
    ...
    findSessionRow(reference) {
        if (typeof reference === "number" || /^\d+$/.test(String(reference))) {
            return this.db.prepare("SELECT * FROM chimera_sessions WHERE id = ?").get(Number(reference));
        }
        return this.db.prepare("SELECT * FROM chimera_sessions WHERE public_id = ? COLLATE NOCASE").get(String(reference));
    }
    configure() {
...
exports.ChimeraStore = ChimeraStore;
...
function messagePublicId(id) {
    return `CM-${String(id).padStart(6, "0")}`;
}
function councilPublicId(id) {
    return `CC-${String(id).padStart(4, "0")}`;
}
function removeChimeraRuntimeForTests(root) {
    const target = node_path_1.default.resolve((0, paths_1.chimeraDbPath)(root));
    const allowed = node_path_1.default.resolve(root, ".argos", "chimera");
    if (!target.startsWith(`${allowed}${node_path_1.default.sep}`))
        throw new Error("Refusing to remove Chimera runtime outside target root");
