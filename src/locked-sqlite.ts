...
const LOCK_WAIT_MS = Number(process.env.ARGOS_SQLITE_LOCK_WAIT_MS ?? 120000);
const LOCK_STALE_MS = Number(process.env.ARGOS_SQLITE_LOCK_STALE_MS ?? 600000);
const RETRY_BASE_MS = 25;
const RETRY_MAX_MS = 250;
const ORPHAN_LOCK_GRACE_MS = 2_000;
const heldLocks = new Map<string, { depth: number; token: string }>();

export function withArgosFileLock<T>(lockDir: string, action: () => T): T {
  const release = new SqliteFileLock(normalizeLockPath(lockDir)).acquire();
  try {
    return action();
  } finally {
    release();
...
export class LockedSqliteDatabase {
  ...
  constructor(dbPath: string) {
    ...
    this.lock = new SqliteFileLock(normalizeLockPath(`${dbPath}.argos-lock`));
  }
...
class SqliteFileLock {
    ...
    while (true) {
      try {
        ...
        if (Date.now() - started > LOCK_WAIT_MS) {
          throw new Error(`Argos SQLite lock timeout after ${LOCK_WAIT_MS}ms: ${this.lockDir}`);
        }
        sleepSync(delay);
  ...
  private removeReusableLock(): void {
    ...
    if (!owner.hasMetadata) {
      if (ageMs < ORPHAN_LOCK_GRACE_MS) return;
      try {
        fs.rmSync(this.lockDir, { recursive: true, force: true, maxRetries: 5, retryDelay: 25 });
      } catch {}
      return;
    ...
    if (ageMs < ORPHAN_LOCK_GRACE_MS && ageMs < LOCK_STALE_MS) return;
    try {
      fs.rmSync(this.lockDir, { recursive: true, force: true, maxRetries: 5, retryDelay: 25 });
    } catch {}
  }
...
function readLockOwner(lockDir: string): { token?: string; pid?: number; mtimeMs: number; hasMetadata: boolean } | null {
  try {
    ...
    return { ...parsed, mtimeMs: stat.mtimeMs, hasMetadata: true };
  } catch {
    try {
      const stat = fs.statSync(lockDir);
      return { mtimeMs: stat.mtimeMs, hasMetadata: false };
    } catch {
      return null;
    }
  }
